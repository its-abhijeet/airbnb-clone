...
function Header({ placeholder }) {
  ...
  return (
    <header className="sticky top-0 z-50 grid grid-cols-3 bg-white shadow-md p-5 md:px-10">
      ...
      <div
        onClick={() => router.push("/")}
        className="relative flex items-center h-100 cursor-pointer my-auto"
      >
      ...
      {searchInput && (
        <div className="flex flex-col col-span-3 mx-auto mt-5">
          ...
          <div className="flex">
            <button
              onClick={resetInput}
              className="flex-grow text-gray-500 hover:scale-105 transition duration-150 ease-in-out"
            >
              Cancel
            </button>
            <button
              onClick={search}
              className="flex-grow text-red-400 hover:scale-105 transition duration-150 ease-in-out"
            >
              Search
            </button>
          </div>
